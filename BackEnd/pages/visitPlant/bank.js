...
export default function BankPage({ callAPI, customAlert }) {
    ...
    useEffect(() => {
        ...
        searchBank(objectReq)
    }, [inputCriteria])

    const searchBank = async (jsonRequest) => {
        if (inputCriteria) {
            jsonRequest.searchOrder = 1
            //inputCriteria.activeFlag = "Y";
            const jsonResponse = await callAPI(apiPath.searchBank, { ...jsonRequest, ...{ model: inputCriteria } })
    ...
    const saveOrUpdate = async () => {
        ...
        if (!inputData.isInvalid) {
            if (initData == null) {
                let jsonRequest = {
                    bankNameTh: inputData.data.bankName,
                    bankNameEn: inputData.data.bankName
                };
                const jsonResponse = await callAPI(apiPath.addBank, jsonRequest)
                ...
                let jsonRequest = {
                    bankId: toString(initData.bankId),
                    bankCode: initData.bankCode,
                    bankNameTh: inputData.data.bankName,
                    bankNameEn: inputData.data.bankName,
                    activeFlag: inputData.data.activeFlag
                };
                const jsonResponse = await callAPI(apiPath.updBank, jsonRequest)
                if (jsonResponse && jsonResponse.errorCode == "S_SUCCESS") {
                    clearInputData(criteriaRef)
                    let criteriaData = getInputData(criteriaRef, "NE");
                    setInputCriteria(criteriaData.data);
    ...
    //         let jsonRequest = {
    //             bankId: toString(item.bankId),
    //             bankCode: toString(item.bankCode),
    //             activeFlag: "N"
    //         }
    //         console.log(jsonRequest)
    //         const jsonResponse = await callAPI(apiPath.cancelBank, jsonRequest)
    //         if (jsonResponse && jsonResponse.errorCode == "S_SUCCESS") {
    //             searchBank(tableRef.current.getJsonReq());
    ...
    return (
        <div className="col-12">
            ...
            >
                <div className="row d-flex justify-content-center">
                    <div className="col-12 col-md-6 mt-2">
                        <TextField label={msg.bankName} value="" ref={el => criteriaRef.current.bankNameTh = el}></TextField>
                    </div>
            ...
            <Modal
                ...
                onClose={() => closeModal()}
                onSave={() => saveOrUpdate()}
                title={msg.headerDialogBank}
            >
                {initData ?
                    <TextField label={msg.bankCode} disabled value={initData && initData.bankCode ? initData.bankCode : ""}></TextField>
                    : null
                }
                <TextField label={msg.bankName} require value={initData && initData.bankNameTh ? initData.bankNameTh : ""} ref={el => inputRef.current.bankName = el}></TextField>
                {initData ?
                    <Switch
