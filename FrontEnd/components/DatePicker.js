...
function Main({
    ...
    useEffect(() => {
        ...
        if (onInit instanceof Function) {
            const returnData = tmpValue ? moment(toString(tmpValue)).format("YYYY-MM-DD").concat("T00:00:00") : null;
            onInit(returnData)
        }
    }, [defaultValue])
    useEffect(() => {
        if (minDate) {
            ...
            if (value instanceof Date) {
                ...
                else if (datetime.isAfter(moment(date).add(lengthDate, unit).format("YYYY-MM-DD").concat("T00:00:00"))) {
                    if (notClearIsAfter) {
    ...
    useImperativeHandle(ref, () => ({
        getData(validateType = "N") {
            ...
            return { isInvalid: validate(value, validateType), value: returnData, isChange: isChange(), label: label };
        },
        clearValue() {
    ...
    function validate(value, validateType = "N") {
        let isEr = false;
        let erMsg = "";
    ...
    function isChange() {
        const date = moment(toString(value));
        const currentValue = date.isValid() ? date.format("YYYY-MM-DD").concat("T00:00:00") : ""
